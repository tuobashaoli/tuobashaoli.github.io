...
msie refresh #对于 MSIE 客户端，该指令可启用发送一个 refresh 头，而不是redirect
```

##### 文件IO指令

```
aio #启用异步文件io，对于linux，需要指定directio，自动禁用sendfile
directio
...
sendfile #直接复制数据从一个到另一个文件描述符
sendfile_max_chunk #在一个snedfile中复制最大数据的大小
```
